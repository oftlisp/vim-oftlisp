...
" std/prelude module.
syn keyword oftlispBuiltins >>= <$> <*> 0? 1+ 1- all and any any-shl? append apply byte? bytes? compare concat concat-strings cons cons? contains? debug-trace each empty? enumerate eq err err? exit exit-with function? gensym geq get greater head float? filter fixnum? foldl foldr group-by id init last length leq less list list? list<-vector lookup lookup-by make-vector map neq nil? not nth ok ok? or or-default ord panic print println put reverse shl? show skip sort sort-by split-at string? string<-symbol string.scalar-head string.scalar-tail symbol? symbol<-string tail take todo vector? vector<-list vector.length vector.nth vector.set vector.slice
